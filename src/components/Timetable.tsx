...
interface Props {
  ...
  onUpdate?: (id: string, patch: Partial<Omit<TimetableEntry, "id">>) => void;
  editMode?: boolean;
}

export function Timetable({ entries, onAdd, onDelete, onArchive, onUpdate, editMode = false }: Props) {
  ...
  return <section className="mt-5 rounded-2xl border border-line bg-surface p-5">
    ...
    <div className="timetable-grid mt-4 grid gap-2 overflow-x-auto sm:grid-cols-7 lg:overflow-visible">
      {days.map((day) => <div key={day.value} className="timetable-day min-w-[110px] rounded-xl border border-line bg-surface-2 p-2 lg:min-w-0"><div className="mb-2 text-xs font-semibold text-muted">{day.short}</div>{visibleEntries.filter((entry) => entry.days.includes(day.value)).sort((a, b) => a.start.localeCompare(b.start)).map((entry) => <div key={entry.id} className="group mb-2 rounded-lg border border-line bg-surface p-2 text-xs">{editMode && onUpdate ? <div className="space-y-1.5"><input defaultValue={entry.subject} onBlur={(e) => onUpdate(entry.id, { subject: e.target.value.trim() || entry.subject })} className="w-full rounded border border-line bg-surface-2 px-1.5 py-1 text-xs font-semibold" /><div className="grid grid-cols-2 gap-1"><input type="time" defaultValue={entry.start} onBlur={(e) => onUpdate(entry.id, { start: e.target.value })} className="rounded border border-line bg-surface-2 px-1 py-1 text-[11px]" /><input type="time" defaultValue={entry.end} onBlur={(e) => onUpdate(entry.id, { end: e.target.value })} className="rounded border border-line bg-surface-2 px-1 py-1 text-[11px]" /></div><input defaultValue={entry.faculty ?? ""} placeholder="Faculty" onBlur={(e) => onUpdate(entry.id, { faculty: e.target.value || undefined })} className="w-full rounded border border-line bg-surface-2 px-1.5 py-1 text-[11px]" /><input defaultValue={entry.room ?? ""} placeholder="Room" onBlur={(e) => onUpdate(entry.id, { room: e.target.value || undefined })} className="w-full rounded border border-line bg-surface-2 px-1.5 py-1 text-[11px]" /></div> : <><div className="font-semibold">{entry.subject}</div><div className="text-muted">{entry.start}–{entry.end}</div>{entry.faculty && <div className="text-muted">{entry.faculty}</div>}{entry.room && <div className="text-muted">{entry.room}</div>}</>}<div className="mt-1 flex gap-2 text-[11px] opacity-0 group-hover:opacity-100"><button type="button" onClick={() => onArchive(entry.id)} className="text-muted">Archive</button><button type="button" onClick={() => onDelete(entry.id)} className="text-flag">Delete</button></div></div>)}</div>)}
    </div>
