...
export function PlannerTools({ mode }: { mode: "daily" | "weekly" | "reminders" }) {
  ...
  const { day, imports, timetable, addBlock, addBlocks, addImport, addReminder, deleteReminder, deleteImport, updateImport, addTimetableEntry, deleteTimetableEntry, archiveTimetableEntry } = useDayRecord();
  ...
  return (
    <main className="mx-auto min-h-screen max-w-[980px] px-5 pb-20 pt-7 sm:px-8">
      ...
      {mode === "weekly" && <Timetable entries={timetable} onAdd={addTimetableEntry} onDelete={deleteTimetableEntry} onArchive={archiveTimetableEntry} />}
